...
ggplot2 is part of the [tidyverse](https://www.tidyverse.org/) set of packages.  The syntax of ggplot2 is a little bit different than other tidyverse packages, in part because it predates them.  But it does share a few things with other tidyverse packages, such as:

* It expects to work with a data frame, and the data frame is the first input
* You can reference the names of columns without quotes or $ syntax within the functions
...
Note that if you want a line or color for each group, those groups need to be defined by a categorical variable, not in separate columns. If the values for the two groups are in separate columns in your dataset, you'll need to reshape your data to get them in a single column instead -- see the [pivot functions in the tidyr package](https://nuitrcs.github.io/r-tidyverse/html/tidyr.html).
